...
def separateStr(strV, seperationSymbols=[",", ";", " ", "\n"], stripStr=True, nonEmpty=True):
  ...
  if not strV:
    ...
    for aSeperator in seperationSymbols:
        strings = FLL(map(lambda x:split(x, aSeperator), strings)) # separate into substrings
        if stripStr: strings = [x.strip() for x in strings] # strip substrings
        if nonEmpty: strings = filter(lambda x: x, strings) # discard empty ones
  return strings
