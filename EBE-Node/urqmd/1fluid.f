...
c     
 ...
 20      v = f
      ...
      if (dabs((v-f)/f).gt.1d-10) goto 10
      velo = f
      return
...
c
       ...
       if (eos.eq.1) then
         entro = 4d0*e*e0/3d0/temp(e,n)/n0
c  
c  If flag eos=[else], use tabellized equation of state.
c     
c     arithmetic mean interpolation (yields better values for
...
c     If eos=1, temperature is that of an ultrarelativistic gas
c     of particles with one degree of freedom.
      
      
      
      if (eos.eq.1) then
         temp = (e*e0*hc3*30d0/pi2)**0.25d0
c     
c     If flag eos=[else], use tabellized equation of state.
c     
...
c
      ...
      SUBROUTINE onefluid(testflag,ithydro)
...
c EoS: 1=ideal gas, 2= HG, 3=Bag model w/PT, 4=chiral w/PT, 5=chiral+HG
c
...
c     if energy loss is greater than 0.1 % then increase dx
         ...
         if(((etot1-etot)/etot1).gt.0.000001d0)then
...
c     
 ...
 300     continue
     ...
     $                          *gamma(i,j,k)
                       ...
                       if((CTOption(52).eq.0).and.(frozen(k).eq.0)) then
                           tf(mfr)=t-dt
                           ifr(mfr)=(float(i)-float(ngr)*.5-.5)*sngl(dx)
                           jfr(mfr)=(float(j)-float(ngr)*.5-.5)*sngl(dx)
                           kfr(mfr)=(float(k)-float(ngr)*.5-.5)*sngl(dx)
     ...
     $                          *gamma(i,j,k)
            ...
            write(*,*) 'total energy after hydro:', etothy*e0*vol/1.d3,
     &           ((etothy*e0*vol/1.d3)-h_etot)/h_etot*100,'%'
            write(*,*)'total entropy after hydro:', entrotote,
...
c               write(*,*) 'evolution is stopped'
            ...
            write(*,*) 'total baryon number after hydro:',
