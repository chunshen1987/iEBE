...
// -- x0: where the interpolation should be performed
{
    ...
    {
//      cout<<"result is"<<endl;
//      cout<<"Pt interp: interp less than zero!"<<endl;  //debug
      result = 0.0e0;
    }
    return result;
...
//**********************************************************************
double BinaryRootFinder(double (*func)(double, void* parameters), void* parameters, double value, double xL, double xR, double precision)
...
{   double x1=xL;
    ...
    while(abs((*func)(xx, parameters)-value)>precision)
    {
      xx=(x1+x2)/2.0;
      f1=(*func)(x1, parameters)-value;
      f2=(*func)(x2, parameters)-value;
      f0=(*func)(xx, parameters)-value;
...
//**********************************************************************************************
int MultiRootFinder(double (*func)(double, void* parameters), void* parameters, double value, double *roots, 
                    int division, double xL, double xR, double precision)
// find multiple roots using binary search method. Divide the region into several smaller ones
{
  ...
  for(int i=0;i<division;i++)
  {
    x1=xL+i*ix;
    x2=x1+ix;
    double f1=(*func)(x1, parameters)-value;
    double f2=(*func)(x2, parameters)-value;

    if(f1*f2<0)    //there is at least one root in [x1,x2]
    {
      roots[idx]=BinaryRootFinder(func, parameters, value,x1,x2,precision);
      idx++;
    }
    ...
    else if(f1*f2>0)   //there is no root within [x1,x2], or two root within this region
    {
      for(int j=0;j<tolerance;j++)
      {
        ...
        double fx=(*func)(xx, parameters)-value;
        if(f1*fx<=0)
        { 
          roots[idx]=BinaryRootFinder(func,parameters, value,x1,xx,precision);
          idx++;
          roots[idx]=BinaryRootFinder(func,parameters, value,xx,x2,precision);
          idx++;
...
// Print out a progress bar with the given percentage. Use a negative value to reset the progress bar.
{
  ...
  // plot status
  int stop;
  if (percentage>=0.99) stop=(int)(0.99*length);
  else stop = (long)(percentage*length);
