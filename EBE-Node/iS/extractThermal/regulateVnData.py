...
def regulateVnData(dir_path, piece=2, reg_path="spectra/thermal_211_integrated_vn.dat", vn_path="spectra/v2data-inte.dat"):
    """
    ...
    for sub_dir in listSubDirectories(dir_path):
        if (not path.exists(path.join(sub_dir, vn_path))):
        ...
        vn_data = readData(path.join(sub_dir, vn_path), "!");
