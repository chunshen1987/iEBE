...
def take(aList, row_l,row_r,col_l,col_r):
...
def fillAll(string, number):
	""" Replace all occurence of %d in the string by number."""
	return string % tuple([number]*string.count("%d"));

def stringContains(string, aList):
	""" Return true if string contains an element in aList as a substring. """
